...
const Header = (props) => {
  ...
  const handleSearch = async (query) => {
    // setIsLoading(true);
    fetch(`https://api.sellship.co/api/searchresults/${props.country}/${query}`)
      .then(async (resp) => {
    ...
    <>
      ...
      <NavBar style={barAnimation} >
        <div className="flex justify-between bg-white h-16 ">
          <Link className="my-auto ml-9 lg:ml-20" to="/">
            <img
          ...
          <AsyncTypeahead className="my-auto flex-initial w-50 sm-50 md:w-60 lg:w-80 xl:w-130 mx-auto shadow-sm  "
            filterBy={filterBy}
            id="async-example"
            // isLoading={isLoading}
            labelKey="searchKey"
          ...
          {props.loggedIn ? (
            ...
            >
              ...
              <button class="bg-purple-700 hover:bg-gray-400 text-gray-800 font-bold py-2 px-4 rounded inline-flex items-center m-2 ">
                ...
                <span className="whitespace-no-wrap text-white">Download App</span>
              </button>


              <Link
                className="transition duration-200 ease-in  mx-2 "
                to="/addItem"
              >
              ...
              <div
                ...
                style={linkAnimation}
              >
                <button class="bg-purple-700 hover:bg-gray-400 text-gray-800 font-bold py-2 px-4 rounded inline-flex items-center m-2 " onClick={showModalDownload}>
                  <svg class="text-white fill-current w-4 h-4 mr-2" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20"><path d="M13 8V2H7v6H2l8 8 8-8h-5zM0 18h20v2H0v-2z" /></svg>
                  <span className="whitespace-no-wrap text-white">Download App</span>
                </button>

                <div class="text-gray-700 text-center bg-gray-400 rounded  text-center px-4 py-2 m-2" onClick={showModalSignIn} style={{cursor: "pointer"}}>Login</div>
                <div class="whitespace-no-wrap bg-purple-700 hover:bg-gray-400 text-white font-bold rounded px-4 py-2 m-2" onClick={showModal} style={{cursor: "pointer"}}>Sign Up</div>
              ...
              >
                ...
                <ul className="dropdown-content absolute hidden text-gray-500 pt-1 bg-white rounded-md  shadow-md">
                  {SUBCATEGORIES[category].map((subcategory, index) => (
                    <li key={index}>
                      <a href={`/#/category/${props.country}/${category}/${subcategory}`}>
                        {subcategory}
                      </a>
                    </li>
                  ))}
